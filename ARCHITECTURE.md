...
10. **Gold** — оверкилл с затуханием (первые 50% maxHP по курсу 1/20,
    дальше 1/40, ×Midas) + точный ласт-хит +5.
...
Башни/боссы — тот же формат: `waves: [{ id, hp, isBoss, modifiers: [{id}] }]`,
модификаторы живут в `content/world.js` и исполняются движком как обычные
триггеры (Aegis = `ON_DEATH → REVIVE`).

## Тесты

`npm test` — раннер грузит те же файлы, что и браузер, в том же порядке в
node:vm и гоняет `tests/*.test.js` внутри контекста. 27 тестов покрывают:
...
оверкилл-затухание, Rapier-цикл, цикл колоды, магазин, сбросы, превью-детерминизм.

## Shop v2, советчик и автосейв

- `systems/economy.js`: 5 предложений `{ id, locked }`, веса редкостей
  62/28/10, реролл сохраняет залоченные.
- `systems/advisor.js`: чистый анализ state — билд-направления для панели
  «ТВОЙ БИЛД» и синергии предметов для «ЛУПА». UI-текст, не движок.
...
- Butterfly-уклонение и BKB захардкожены в фазе tower mods, не вынесены в
  контент-триггеры (TODO при появлении боссов с дебаффами).
