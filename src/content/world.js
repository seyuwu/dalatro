...
const FORMATIONS_DATA = [
  {
    ...
    example: ["tidehunter"],
    desc: "Урон скромный, зато рука почти не тратится и капает +1 золота: так дешевле всего добивать башню, оставшуюся с прошлых боёв.",
  },
  {
    ...
    example: ["huskar", "ursa"],
    desc: "Тратит меньше руки, чем пятёрка: удобно щипать башню парами, когда больших боёв на волну не осталось.",
  },
  // Фолбэк: любой отряд 3+ всегда что-то наносит (аналог «старшей карты»).
  {
    ...
    desc: "Страховка от пустых боёв. Как только отряд складывается в настоящую формацию, она перебивает Отряд по урону.",
  },
  {
    ...
    desc: "Например Сила + Ловкость + Интеллект — Универсал тоже считается. Бьёт магией: это выход против башен с высокой бронёй.",
  },
  {
    ...
    desc: "За фронтом — кто угодно. Порядок решает: поставь на фронт Силовика слабее 6, и Стена не соберётся.",
  },
  {
    ...
    desc: "У троих центр — слот 2, у пятёрки — слот 3, у четвёрки — один из двух средних. Чистый урон игнорирует броню и сопротивление.",
  },
  {
    ...
    desc: "Одна перестановка ломает лесенку, так что Рампа — проверка аккуратности с множителем почти как у Клина.",
  },
  ...
  // тройке «клещи» вырождаются в «минимум в центре» и перехватывают всё.
  {
    ...
    desc: "Зеркало Клина: сильные герои по краям берут слабую середину в клещи, как в примере 9-3-7-10.",
  },
  ...
  // Гейт 4+ по той же причине: на тройке это просто «пара через слот».
  {
    ...
    desc: "Например 4-7-7-4. Нужны повторяющиеся силы — пары героев одного ранга или тренировка, поэтому собирается поздно.",
  },
  {
    ...
    desc: "Монолит одного цвета. Тип урона берётся от него: Сила и Ловкость бьют физикой, Интеллект — магией, Универсал — чистым уроном.",
  },
  ...
  // любую руку с пиком в центре и убивает A/B-тест гипотезы.
  {
    ...
    desc: "Кэрри занимает слот 3. Свите не нужно быть сильной — она тащит связки и способности, пока кэрри разносит башню.",
  },
  {
    ...
    desc: "Формация-топ: рекордные база и множитель. Порядок слотов не важен — важен сам набор сил, так что собирай её всегда, когда рука позволяет.",
  },
];
