        ...
        if (resolution.killed) {
          ...
          if (s.combat.wave.isBoss) {
            if (s.run.waveIndex >= Content.waves.order.length - 1) {
              ...
              // Акт пройден: премия за переход — скромное золото (главная
              // награда — казарма) и восстановление казармы.
              s.run.gold += 5;
              ...
              log(s, `АКТ ${s.run.act} ПРОЙДЕН! +5 золота${perAct ? ` (+${perAct} за кошелёк)` : ""}, +1 казарма (восстановление)`);
